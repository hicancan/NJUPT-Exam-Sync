...
from .validation.hot_query_contract import validate_high_df_hot_query_proofs, validate_hot_query_fast_start
from .validation.public_artifacts import artifact_path, ensure_no_obsolete_fields as ensure_no_obsolete_fields_raw, ensure_public_hashed_path, fail, read_json
from .validation.source_manifest_expansion import load_proof_catalogs, load_source_manifests, proof_catalog_shards
...
def ensure_no_obsolete_fields(payload: Any, path: str = "$") -> None:
    ensure_no_obsolete_fields_raw(payload, OBSOLETE_FIELDS, path)
...
def validate_local_indexes(source_manifests: list[dict[str, Any]], expected_source_ids: set[str]) -> list[dict[str, Any]]:
    ...
    for source_manifest in source_manifests:
        ...
        for name in ("local_indexes", "proof_catalog", "shard_filter", "attachment_meta_index", "attachment_filename_index", "attachment_text_shards"):
...
def validate_generated_index(packages: list[dict[str, Any]] | dict[str, Any]) -> dict[str, Any]:
    ...
    validate_high_df_hot_query_proofs(manifest)
    ...
    for item in sources:
        ...
        if item.get("coverage_evidence_source") != "full_crawl":
            fail(
                "source registry coverage_evidence_source must be full_crawl: "
                f"{item.get('source_id')}={item.get('coverage_evidence_source')!r}"
            )
