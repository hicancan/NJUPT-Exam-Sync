...
import { parseSpaceGeometry, parseSpaceManifest, SpaceContractError } from './index';
...
describe('SpaceSnapshot decoder', () => {
    it('accepts only the current content-addressed manifest shape', () => {
        const manifest = parseSpaceManifest({
            format: 'njupt-space-snapshot', snapshot_id: hash('b'), source_id: hash('c'),
            campus_count: 3, building_count: 11, floor_count: 41, space_family_count: 531,
            space_unit_count: 598, geometry_unit_count: 356, unresolved_count: 28,
            artifacts: {
                campuses: artifact('campuses.json'), buildings: artifact('buildings.json'), floors: artifact('floors.json'),
                space_families: artifact('space-families.json'), space_units: [artifact('space-units-building.json')],
                aliases: artifact('aliases.json'), connectors: artifact('connectors.json'), geometry: [artifact('geometry-floor.json')], audit: artifact('audit.json'),
            },
        });
        expect(manifest.space_family_count).toBe(531);
        expect(() => parseSpaceManifest({ ...manifest, room_catalog_id: hash('d') })).toThrow(SpaceContractError);
    });
