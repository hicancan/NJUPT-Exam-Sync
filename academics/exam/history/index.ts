...
export const parseExamHistoryManifest = (payload: unknown, source = 'exam history manifest'): ExamHistoryManifest => {
    ...
    if (payload.version !== 'exam-history-manifest-v2') {
    ...
    const classes = requireArray<Record<string, unknown>>(payload, 'classes', source);
    const seen = new Set<string>();
    for (const [index, entry] of classes.entries()) {
        if (!isObject(entry)) {
            throw new Error(`${source} does not match exam history manifest contract: classes[${index}] must be an object`);
        }
        const classKey = requireString(entry, 'class_key', `${source}.classes[${index}]`);
        const artifact = requireObject(entry, 'artifact', `${source}.classes[${index}]`);
        if (
            typeof artifact.path !== 'string'
            || artifact.path !== `history/classes/${classKey}.json`
            ...
            throw new Error(`${source} does not match exam history manifest contract: invalid class artifact`);
        }
        seen.add(classKey);
    }
