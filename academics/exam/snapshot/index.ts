...
export const parseManifest = (payload: unknown, source = 'data summary'): Manifest => {
    ...
    if (
        payload.format !== 'njupt-exam-snapshot-v2'
        || typeof payload.snapshot_id !== 'string'
        || !SHA256_RE.test(payload.snapshot_id)
    ) {
        throw new DataContractError(`Validation failed for ${source}: incompatible ExamSnapshot identity`);
    }
    ...
    if (!isPlainObject(payload.artifacts)) {
        throw new DataContractError(`Validation failed for ${source}: artifacts must be an object`);
    }
    const expectedArtifacts = {
        records: 'exams.json',
        class_index: 'class-index.json',
    ...
    for (const [name, expectedPath] of Object.entries(expectedArtifacts)) {
        ...
        if (
            ...
            throw new DataContractError(`Validation failed for ${source}: invalid ${name} artifact`);
        }
    }
...
export const parseExamClassIndex = (payload: unknown, source = 'exam class index'): ExamClassIndex => {
    ...
    if (value.version !== 'exam-class-index-v2') throw new DataContractError(`Validation failed for ${source}: invalid version`);
    ...
    for (const item of value.classes) {
        ...
        const refs = [
            [item.data, /^classes\/[^/]+\.json$/, 'class data'],
            [item.history, /^history\/classes\/[^/]+\.json$/, 'class history'],
        ] as const;
        for (const [artifact, pathPattern, description] of refs) {
            if (
                !isPlainObject(artifact)
                || typeof artifact.path !== 'string'
                || !pathPattern.test(artifact.path)
                || !Number.isSafeInteger(artifact.bytes)
                || Number(artifact.bytes) <= 0
                || typeof artifact.sha256 !== 'string'
                || !SHA256_RE.test(artifact.sha256)
            ) {
                throw new DataContractError(`Validation failed for ${source}: invalid ${description} artifact`);
            }
        }
...
export const examSnapshotIdentityText = (manifest: Manifest): string => {
    const parts = [manifest.format, manifest.data_version];
    for (const name of Object.keys(manifest.artifacts).sort()) {
        const artifact = manifest.artifacts[name as keyof Manifest['artifacts']];
        parts.push(name, artifact.path, String(artifact.bytes), artifact.sha256);
    }
    return parts.join('\0');
};

export const assertExamSnapshotIdentity = async (manifest: Manifest): Promise<void> => {
    if (await digestHex(examSnapshotIdentityText(manifest)) !== manifest.snapshot_id) {
        throw new DataContractError('Validation failed for ExamSnapshot: content identity mismatch');
    }
};
...
export const resolveExamDataVersion = (manifest: Manifest): string => {
    ...
    if (!dataVersion) {
        throw new DataContractError('ExamSnapshot data_version is required');
    }
    return dataVersion;
};
...
export const assertManifestMatchesExams = (manifest: Manifest, exams: Exam[]) => {
    if (manifest.total_records !== exams.length) {
        throw new DataContractError(
            `ExamSnapshot total_records=${manifest.total_records} does not match exams.length=${exams.length}`
        );
    }
    for (const exam of exams) {
...
export const assertClassIndexMatchesManifest = (manifest: Manifest, classIndex: ExamClassIndex) => {
    if (classIndex.data_version !== manifest.data_version) {
        throw new DataContractError('class index data_version does not match ExamSnapshot data_version');
    }
    if (classIndex.exam_period_id !== manifest.exam_period_id) {
        throw new DataContractError('class index exam_period_id does not match ExamSnapshot exam_period_id');
    }
    if (classIndex.total_records !== manifest.total_records) {
        throw new DataContractError('class index total_records does not match ExamSnapshot total_records');
    }
};
