...
export function ExamHistorySummary({ manifest, events, loading, error }: ExamHistorySummaryProps) {
    if (loading) {
        return <p className="mt-5 text-center text-[13px] text-[#70757a] dark:text-[#9aa0a6]">正在读取更新记录…</p>;
    }
    if (error) {
        return <p className="mt-5 text-center text-[13px] text-[#b3261e] dark:text-[#f28b82]">{error}</p>;
    }
    const latest = events?.events[events.events.length - 1];
    if (!manifest || !latest) return null;
    return (
        <section className="mx-auto mt-5 max-w-[692px] rounded-xl border border-[#dadce0] bg-white px-5 py-4 text-left dark:border-[#3c4043] dark:bg-[#202124]" aria-label="考试安排更新摘要">
