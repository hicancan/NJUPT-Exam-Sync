            ...
            {coverage && showDiagnostics ? (
                <div className="mt-2 max-w-[880px] rounded-md border border-[#dadce0] dark:border-[#3c4043] bg-[#f8fafc] dark:bg-[#2d2e30] px-3 py-2 text-[13px] text-[#4d5156] dark:text-[#bdc1c6]">
                    <div className="flex flex-wrap gap-x-4 gap-y-1 [&>span]:max-w-full [&>span]:break-all">
                        <span>已验证跳过 {coverage.proved_no_match_shards}</span>
                        <span>筛选排除 {coverage.excluded_by_filter_shards}</span>
                        <span>待扫描 {coverage.pending_shards}</span>
                        ...
                        <span>首次加载 {formatBytes(coverage.uncached_loaded_bytes)}</span>
                        ...
                        <span>阶段：{coverage.phase === 'scoped_exhaustive_complete' || coverage.phase === 'global_exhaustive_complete' ? '搜索完成' : coverage.phase}</span>
                        ...
                        {queryStats ? (
                            <>
                                ...
                                <span>快速启动 {queryStats.fast_start_used ? '是' : '否'}</span>
                                <span>本地元数据补充 {queryStats.fallbacks.localMetaFallbackDocuments}</span>
                                <span>摘要补充 {queryStats.fallbacks.snippetFallbackResults}</span>
                                <span>验证命中 {queryStats.fallbacks.verifiedFullScanMatches}</span>
                                <span>本地索引 {queryStats.loadedLocalIndexCount}</span>
                                ...
                                <span>跳过倒排记录 {queryStats.retrieval.postingsPruned}</span>
                            </>
                        ) : null}
                    </div>
                </div>
